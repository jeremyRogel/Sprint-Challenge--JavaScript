...
console.log(stegosuarus.length);
...
const contactInfo = [];
  graduates.forEach(function(items) {
    return constactinfo.push(`${items.first_name}` + " " + `${items.email}`)
  })
...
zooAnimals.forEach((items) => {
  return lowCaseAnimalNames.push(`${items.animal_name} ${items.scientific_name}`)
});
